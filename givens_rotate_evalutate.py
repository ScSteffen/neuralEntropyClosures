...
def main():
    ...
    if degree == 1:
        ...
        model = init_neural_closure(network_mk=11, poly_degree=degree, spatial_dim=2, folder_name="tmp",
                                        ...
                                        input_decorrelation=True, scale_active=False, gamma_lvl=2, basis="spherical_harmonics")
        model.load_model("models/Harmonic_models/Harmonic_Mk11_M2_2D_gamma0/")
        alpha_size = 5
...
def tester2D(alpha1: np.ndarray, alpha2: np.ndarray, et2,mk11_m2_2d_g0,count):
    ...
    h_res, alpha_res, u_rot_batch = rotate_compute_M2_network(u_batch, mk11_m2_2d_g0,et=et2)
    ...
    for i in range(n_iterpolators):
        if h_res[i] > lambdas[i]*h_res[n_iterpolators-1] + (1-lambdas[i])*h_res[0]:# and h_res[i]< 20.0:
            ...
            u_temp = u_batch[:,1:]
            for i in range(n_iterpolators):
                plt.plot(u_temp[i, 0], u_temp[i, 1], 'ro')
                plt.plot(u_rot_batch[i, 0], u_rot_batch[i, 1], 'k*')
            plt.xlim([-2,2])
            plt.ylim([-2,2])
            plt.savefig("test_imgs/test_" +str(count) + "_u_1.png")
            plt.clf()
            
            for i in range(n_iterpolators):
                plt.plot(u_temp[i, 0], u_temp[i, 2], 'ro')
                plt.plot(u_rot_batch[i, 0], u_rot_batch[i, 2], 'k*')
            plt.xlim([-2,2])
            plt.ylim([-2,2])
            plt.savefig("test_imgs/test_" +str(count) + "_u_2.png")
            plt.clf()
            ...
            plt.xlim([-2,2])
            plt.ylim([-2,2])
            plt.savefig("test_imgs/test_" +str(count) + "_u_3.png")
            plt.clf()
            ...
            plt.xlim([-2,2])
            plt.ylim([-2,2])
            plt.savefig("test_imgs/test_" +str(count) + "_u_4.png")
            plt.clf()
...
def rotate_compute_M1_network(u_batch: np.ndarray, mk11_m2_2d_g0,et ):
    ...
    u_mirr_batch = np.zeros(shape=(u_batch.shape[0], u_batch.shape[1]))
...
def rotate_compute_M2_network(u_batch: np.ndarray,mk11_m2_2d_g0,et ):
    """
    brief: Rotates to x-line, evaluates, rotates back
    """
    ...
    # alpha_res_full = et.reconstruct_alpha(tf.constant(alpha_res,dtype=tf.float64))
    # h_res = et.compute_h(tf.constant(u_batch,dtype=tf.float64),alpha_res_full).numpy()
    return h_res, alpha_res, u_rot_batch[:,1:]
