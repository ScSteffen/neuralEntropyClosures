...
anim = @animate for iter = 1:nt
    ...
    @inbounds for i = 1:nx
        phi1[:, i] .= KitBase.realizable_reconstruct(α1[:, i], m, weights, KitBase.maxwell_boltzmann_dual_prime)
    end
    ...
    plot(ps.x, (abs.(phi[1, :] .- phi1[1, :]) .+ abs.(phi[2, :] .- phi1[2, :])))
end

gif(anim, "compare_l1error.gif", fps = 45)
