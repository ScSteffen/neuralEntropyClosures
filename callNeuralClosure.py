...
def main():
    ...
    parser.add_option(
        "-b", "--batch", dest="batch", default=128, help="batch size", metavar="BATCH"
    )
    parser.add_option(
        "-c",
        "--curriculum",
    ...
    parser.add_option(
        ...
        dest="epoch",
        default=1000,
        help="epoch count for neural network",
        metavar="EPOCH",
    )
    parser.add_option(
        "-f",
    ...
    parser.add_option(
        ...
        dest="scaledOutput",
        default="0",
        help="train on scaled entropy values",
        metavar="SCALEDOUTPUT",
    )
    parser.add_option(
        "-i",
        "--decorrInput",
        dest="decorrInput",
        default="0",
        help="train normalized and decorrelated input moments",
        metavar="SCALEDINPUT",
    )
    parser.add_option(
        "-l",
    ...
    parser.add_option(
        ...
        dest="normalized",
        default=0,
        help="train on normalized moments",
        metavar="NORMALIZED",
    )
    parser.add_option(
        "-o",
    ...
    parser.add_option(
        ...
        dest="processingmode",
        default=1,
        help="gpu mode (1). cpu mode (0) ",
        metavar="PROCESSINGMODE",
    )
    parser.add_option("-r", "--rotated", dest="rotated", default=0)
    parser.add_option(
        "-s",
        "--spatialDimension",
        dest="spatialDimension",
        default=3,
        help="spatial dimension of closure",
        metavar="SPATIALDIM",
    )
    parser.add_option(
        "-t",
    ...
    parser.add_option(
        ...
        dest="verbosity",
        default=1,
        help="output verbosity keras (0 or 1)",
        metavar="VERBOSITY",
    )
    parser.add_option(
        "-w",
        ...
        dest="networkwidth",
        default=10,
        help="width of each network layer",
        metavar="WIDTH",
    )
    parser.add_option(
        "-x",
    ...
    neuralClosureModel = init_neural_closure(
        ...
        nw_width=options.networkwidth,
        nw_depth=options.networkdepth,
        input_decorrelation=options.decorrInput,
        scale_active=options.scaledOutput,
        gamma_lvl=options.gamma_level,
        basis=options.basis,
    ...
    if options.training == 1:
        ...
        neuralClosureModel.load_training_data(
            shuffle_mode=True,
    ...
    if (
        ...
        neuralClosureModel.training_data_preprocessing(
            scaled_output=options.scaledOutput, model_loaded=options.loadmodel
        )
    else:
    ...
    if options.training == 1:
        ...
        print(
            "Re-Save mode entered."
        )  # if training was not finished, models are not safed to .pb. this can be done here
        neuralClosureModel.load_training_data(
            shuffle_mode=False,
            sampling=options.sampling,
            normalized_data=neuralClosureModel.normalized,
